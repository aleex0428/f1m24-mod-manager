      ...
      <div className="mx-auto max-w-3xl space-y-4 px-6 py-6">
        ...
        <section className="panel p-5">
          ...
          <div className="flex flex-wrap gap-2">
            <input
              ...
              className="input min-w-[16rem] flex-1 cursor-default font-mono !text-xs text-text-secondary"
            />
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3 border-t border-border-subtle pt-4">
            ...
            <button onClick={handleOpenModsFolder} disabled={!modsPath} className="btn-ghost !py-2 !text-xs">
              Open folder
            </button>
          </div>
        </section>
        ...
        <section className="panel p-5">
          ...
          <p className="mb-4 mt-0.5 text-sm text-text-muted">
            Archives (.zip / .rar / .7z) are downloaded to a temporary folder, unpacked, and only the
            .pak files are copied into the game. A copy of each archive is kept here so you can
            reinstall without downloading again.
          </p>

          <div className="flex flex-wrap items-center gap-3">
            <p className="min-w-0 flex-1 truncate font-mono text-xs text-text-secondary" title={archivesPath}>
              {archivesPath || "—"}
            </p>
        ...
        <section className="panel p-5">
          ...
          <ul className="mt-4 space-y-1.5 border-t border-border-subtle pt-4 text-xs text-text-muted">
            ...
            <li>
              Tick <span className="text-text-secondary">“Stay signed in”</span> on the login form.
              Without it the session is dropped when you close the app.
            </li>
          </ul>
        </section>
        ...
        <section className="panel p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Stat label="Last sync" value={stats?.lastSync ? formatDate(stats.lastSync) : "Never"} />
          </div>
        </section>

        {/* Behaviour */}
        <section className="panel divide-y divide-border-subtle">
          <PreferenceRow
        ...
        <section className="panel p-5">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => invoke("quit_app").catch(() => {})} className="btn-ghost !py-2 !text-xs">
                Quit app
              </button>
            </div>
          </div>
          ...
          {update.stage === "available" && update.notes && (
            <p className="mt-3 max-h-24 overflow-y-auto whitespace-pre-wrap rounded-xl border border-border-subtle bg-bg-elevated p-3 text-xs text-text-secondary">
              {update.notes}
            </p>
          )}
